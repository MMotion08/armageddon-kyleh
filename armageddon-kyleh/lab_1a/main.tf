...
# Explanation: balosar needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
resource "aws_vpc" "balosar_vpc01" {
...
resource "aws_internet_gateway" "balosar_igw01" {
  vpc_id = aws_vpc.balosar_vpc01.id
...
# Explanation: balosar wants the private base to call home—EIP gives the NAT a stable “holonet address.”
resource "aws_eip" "balosar_nat_eip01" {
  domain = "vpc"
...
# Explanation: NAT is balosar’s smuggler tunnel—private subnets can reach out without being seen.
resource "aws_nat_gateway" "balosar_nat01" {
  allocation_id = aws_eip.balosar_nat_eip01.id
  ...
  depends_on = [aws_internet_gateway.balosar_igw01]
}
...
resource "aws_route_table" "balosar_public_rt01" {
  vpc_id = aws_vpc.balosar_vpc01.id
...
resource "aws_route" "balosar_public_default_route" {
  route_table_id         = aws_route_table.balosar_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.balosar_igw01.id
}
...
resource "aws_route_table" "balosar_private_rt01" {
  vpc_id = aws_vpc.balosar_vpc01.id
...
resource "aws_route" "balosar_private_default_route" {
  route_table_id         = aws_route_table.balosar_private_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.balosar_nat01.id
}
...
resource "aws_security_group" "balosar_ec2_sg01" {
...
resource "aws_vpc_security_group_ingress_rule" "balosar_ec2_sg_ingress_http" {
  ...
  security_group_id = aws_security_group.balosar_ec2_sg01.id
  from_port         = local.ports_http
  to_port           = local.ports_http
  cidr_ipv4         = local.all_ip_address
}

resource "aws_vpc_security_group_ingress_rule" "balosar_ec2_sg_ingress_ssh" {
  ip_protocol       = local.tcp_protocol
  security_group_id = aws_security_group.balosar_ec2_sg01.id
...
resource "aws_security_group" "balosar_rds_sg01" {
...
# TODO: student adds inbound MySQL 3306 from aws_security_group.balosar_ec2_sg01.id

resource "aws_vpc_security_group_ingress_rule" "balosar_rds_sg_ingress_mysql" {
  ip_protocol                  = local.tcp_protocol
  security_group_id            = aws_security_group.balosar_rds_sg01.id
  from_port                    = local.db_port
  to_port                      = local.db_port
  referenced_security_group_id = aws_security_group.balosar_ec2_sg01.id #allow traffic ONLY from specified SG
}
...
resource "aws_db_subnet_group" "balosar_rds_subnet_group01" {
...
resource "aws_db_instance" "balosar_rds01" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.balosar_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.balosar_rds_sg01.id]
...
# Explanation: balosar refuses to carry static keys—this role lets EC2 assume permissions safely.
resource "aws_iam_role" "balosar_ec2_role01" {
...
resource "aws_iam_role_policy_attachment" "balosar_ec2_ssm_attach" {
  role       = aws_iam_role.balosar_ec2_role01.name
...
resource "aws_iam_instance_profile" "balosar_instance_profile01" {
...
resource "aws_instance" "balosar_ec201" {
  ami                         = var.ec2_ami_id
  instance_type               = var.ec2_instance_type
  subnet_id                   = aws_subnet.balosar_public_subnets[0].id
  vpc_security_group_ids      = [aws_security_group.balosar_ec2_sg01.id]
  iam_instance_profile        = aws_iam_instance_profile.balosar_instance_profile01.name
  user_data_replace_on_change = true

  # TODO: student supplies user_data to install app + CW agent + configure log shipping
  user_data  = file("${path.module}/user_data.sh")
  depends_on = [aws_db_instance.balosar_rds01]
...
resource "aws_ssm_parameter" "balosar_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.balosar_rds01.port)
...
resource "aws_secretsmanager_secret" "balosar_db_secret01" {
...
resource "aws_secretsmanager_secret_version" "balosar_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.balosar_db_secret01.id

  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.balosar_rds01.address
    port     = aws_db_instance.balosar_rds01.port
    dbname   = var.db_name
  })
}
...
resource "aws_cloudwatch_log_group" "balosar_log_group01" {
...
# Explanation: Metrics are balosar’s growls—when they spike, something is wrong.
# NOTE: Students must emit the metric from app/agent; this just declares the alarm.
resource "aws_cloudwatch_metric_alarm" "balosar_db_alarm01" {
  ...
  alarm_actions = [aws_sns_topic.balosar_sns_topic01.arn]
...
resource "aws_sns_topic" "balosar_sns_topic01" {
...
resource "aws_sns_topic_subscription" "balosar_sns_sub01" {
  topic_arn = aws_sns_topic.balosar_sns_topic01.arn
  protocol  = "email"
...
# resource "aws_vpc_endpoint" "balosar_vpce_ssm" { ... }